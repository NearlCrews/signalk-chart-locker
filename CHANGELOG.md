...
- **Basemap in a saved region (fully offline).** The vector basemap is now a selectable source when
  saving a region, so a downloaded region renders its base layer offline: geometry, labels, and icons.
  The basemap tiles warm at their native vector maxzoom and overzoom above it; the font glyphs (common
  scripts) and the sprite warm once globally and every region reuses them. The basemap is offered for a
  region download only, never for position warm.
